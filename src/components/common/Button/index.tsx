...
const Button: React.FC<ButtonProps> = React.forwardRef((props, ref: any) => {
  ...
  const defaultTextStyles: StyleProp<TextStyle> = {
    ...
    fontSize: CONSTANT.BUTTON.fontSize,
    color: textColor,
    fontWeight: CONSTANT.BUTTON.fontWeight as TextStyle['fontWeight'],
    alignSelf: 'center',
    opacity: loading ? 0 : 1,
  ...
  const leftIconStyles: StyleProp<TextStyle> = [
    ...
    {
      margin: 0,
      marginRight: title ? -((otherTextStyle.margin as number) / 2) : 0,
      marginLeft: title ? otherTextStyle.margin : 0,
    },
    leftIconProps?.style,
  ];
