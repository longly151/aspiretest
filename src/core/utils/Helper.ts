...
class CHelper {
  ...
  currencyFormat = (value: number, showSymbol = false) => {
  ...
  numberFormat = (currency: string) => Number(currency.replace(/[^0-9.-]+/g, ''));
  ...
  inputCurrencyFormat = (currencyString: string, showSymbol = false) => {
    const numberText = Helper.removeUselessCharacters(currencyString, ['-', ',', ' ', '\\$']);
    const symbol = showSymbol ? '$' : '';
    if (!numberText) return symbol;
